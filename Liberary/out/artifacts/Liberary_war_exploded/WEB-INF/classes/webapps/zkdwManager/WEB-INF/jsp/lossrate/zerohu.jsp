...
<body>
		<div class="center">
			<div class="search"> 
				<form id="zerohuform" class="form-inline" role="form" action="<%=basePath%>lossrateAct/zerohu.html" method="post">
				  <div class="form-group">
					<label>抄表段编号</label>
					<input type="text" name="searchItems" value="${searchItems }" class="form-control" readonly="readonly" placeholder="台区名称/编码">
...
<!-- 							<th>操作人</th> -->
			...
			<form action="<%=basePath%>lossrateAct/zerohu.html" method="post" id="pageSizee">
...
<!-- 				  <option value="20" <c:if test="${pagenation.pageSize ==20 }">selected="selected"</c:if>>20</option> -->
			   ...
			   <div class="fr">
			   <form action="<%=basePath%>lossrateAct/zerohu.html" method="post" onsubmit="return topage();">
				 <ul class="page-right">
					<li ><a href="<%=basePath%>lossrateAct/zerohu.html?pageNo=1&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&userType=${userType}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}">首页</a></li>
					<li class="previous">
					<a href="<%=basePath%>lossrateAct/zerohu.html?pageNo=${pagenation.pageNo-1}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&userType=${userType}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}" onclick="return prevPage(${pagenation.pageNo})">上一页</a></li>
					<li>
					<input type="text" value="${pagenation.pageNo}" name="pageNo">
					<input name="pageSize" type="hidden" value="${pagenation.pageSize}">
					<input type="hidden" name="searchItems" value="${searchItems }" class="form-control" placeholder="台区名称/编码">
				    <input type="hidden" name="userType" value="${userType }">
				    <input type="hidden" name="endYear" value="${endYear }">
				  <input type="hidden" name="endMonth" value="${endMonth }">
				  <input type="hidden" name="startMonth" value="${startMonth }">
				  <input type="hidden" name="startYear" value="${startYear }">
					...
					<li class="next">
					<a href="<%=basePath%>lossrateAct/zerohu.html?pageNo=${pagenation.pageNo+1}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&userType=${userType}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}" onclick="return nextPage(${pagenation.pageNo},${totalPage })">下一页</a></li>
					<li><a href="<%=basePath%>lossrateAct/zerohu.html?pageNo=${totalPage}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&userType=${userType}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}">尾页</a></li>
				 </ul>
				 </form>
			   </div>		 
			</div>	
...
function exports(){
	...
	$("#zerohuform").attr("action",basePath + "ExportexcleAct/zerohuexport.html");
	$("#zerohuform").submit();
	$("#zerohuform").attr("action",basePath + "lossrateAct/zerohu.html");
}
</script>
</html>