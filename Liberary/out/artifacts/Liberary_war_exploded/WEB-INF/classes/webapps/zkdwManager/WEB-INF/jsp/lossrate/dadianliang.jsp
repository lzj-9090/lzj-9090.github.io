...
<body>
		<div class="center">
			<div class="search"> 
				<form id="zerohuform" class="form-inline" role="form" action="<%=basePath%>lossrateAct/dadianliang.html" method="post">
...
<!-- 							<th>操作人</th> -->
			...
			<form action="<%=basePath%>lossrateAct/dadianliang.html" method="post" id="pageSizee">
...
<!-- 				  <option value="20" <c:if test="${pagenation.pageSize ==20 }">selected="selected"</c:if>>20</option> -->
			   ...
			   <div class="fr">
			   <form action="<%=basePath%>lossrateAct/dadianliang.html" method="post" onsubmit="return topage();">
				 <ul class="page-right">
					<li ><a href="<%=basePath%>lossrateAct/dadianliang.html?pageNo=1&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&is=${is}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}">首页</a></li>
					<li class="previous">
					<a href="<%=basePath%>lossrateAct/dadianliang.html?pageNo=${pagenation.pageNo-1}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&is=${is}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}" onclick="return prevPage(${pagenation.pageNo})">上一页</a></li>
					<li>
				 ...
				 <input type="hidden" name="endYear" value="${endYear }">
				  ...
				  <input type="hidden" name="startYear" value="${startYear }">
					...
					<li class="next">
					<a href="<%=basePath%>lossrateAct/dadianliang.html?pageNo=${pagenation.pageNo+1}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&is=${is}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}" onclick="return nextPage(${pagenation.pageNo},${totalPage })">下一页</a></li>
					<li><a href="<%=basePath%>lossrateAct/dadianliang.html?pageNo=${totalPage}&pageSize=${pagenation.pageSize}&searchItems=${searchItems}&is=${is}&endYear=${endYear}&endMonth=${endMonth}&startMonth=${startMonth}&startYear=${startYear}">尾页</a></li>
				 </ul>
				 </form>
			   </div>		 
			</div>	
...
function exports(){
	...
	$("#zerohuform").attr("action",basePath + "ExportexcleAct/dadianliangexport.html");
	$("#zerohuform").submit();
	$("#zerohuform").attr("action",basePath + "lossrateAct/dadianliang.html");
}
</script>
</html>